...
class Order extends BaseModel
{
    ...
    public static function makeOne(OrderPurchase $purchase, array $data)
    {
        ...
        $cart = \App\Facades\Cart::getClassName()::byToken(Arr::get($data, 'cart_token'))->first();

        $customerForeignKey = \App\Facades\Customer::getForeignKey();

        $order->{$customerForeignKey} = $purchase->{$customerForeignKey};
        $order->{\App\Facades\Coupon::getForeignKey()} = $cart->{\App\Facades\Coupon::getForeignKey()};
        ...
        $order->{\App\Facades\Cart::getForeignKey()} = $cart->id;
